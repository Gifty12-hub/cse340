const utilities = require("../utilities")  // ← was "." — now correctly points to utilities/index.js
...
validate.checkUpdateData = async (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    ...
    return res.render("account/update", {
      errors, title: "Update Account Information", nav,
      user, account_firstname, account_lastname, account_email,
    })
  }
  next()
}
...
validate.updatePasswordRules = () => {
  return [
    body("new_password").trim().notEmpty()
      .isStrongPassword({ minLength: 12, minLowercase: 1, minUppercase: 1, minNumbers: 1, minSymbols: 1 })
      .withMessage("Password must be at least 12 characters with uppercase, number, and special character."),
  ]
}
...
validate.checkUpdatePasswordData = async (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    ...
    const user = await accountModel.getAccountById(account_id)
    return res.render("account/update", {
      errors, title: "Update Account Information", nav, user,
    })
  }
  next()
}
